...
**Transfer options:**
- **Do nothing (recommended default):** let the passkeys go.  Passwords migrated via the script route, so when a user next visits a site they log in with the password and save a fresh passkey to Bitwarden on the spot.  No inventory, no coordination, no lockout risk.  See docs/DECISIONS.md (2026-09-05).
- **Systematic per-site re-registration (optional):** search `=passkey` in the 1Password app — that live search is the checklist.  Re-register each site into Bitwarden, then delete the passkey from the 1Password item so it drops out of the search.  When `=passkey` shows zero, you are done.
